...
from ee.clickhouse.client import format_sql, sync_execute, ch_client, substitute_params
...
class ClickhouseTrendsFormula:
    def _run_formula_query(self, filter: Filter, team_id: int):
        ...
        """.format(
            ...
            queries=
            ''.join(['FULL OUTER JOIN ({query}) as sub_{letter} ON sub_A.breakdown_value = sub_{letter}.breakdown_value '.format(query=query, letter=letters[i+1]) for i, query in enumerate(queries[1:])])
            if filter.breakdown else
            ''.join(['CROSS JOIN ({}) as sub_{}'.format(query, letters[i+1]) for i, query in enumerate(queries[1:])])
        )
