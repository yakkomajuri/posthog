...
from posthog.constants import TRENDS_CUMULATIVE, TRENDS_LIFECYCLE, TREND_FILTER_TYPE_ACTIONS
...
class ClickhouseTrends(ClickhouseTrendsNormal, ClickhouseTrendsBreakdown, ClickhouseLifecycle, ClickhouseTrendsFormula, Trends):
    ...
    def _get_sql_for_entity(self, filter: Filter, entity: Entity, team_id: int) -> Tuple[str, Dict, Callable]:
        if filter.breakdown:
            sql, params, parse_function = self._serialize_breakdown(entity, filter, team_id)
        elif filter.shown_as == TRENDS_LIFECYCLE:
            sql, params, parse_function= self._serialize_lifecycle(entity, filter, team_id)
        else:
