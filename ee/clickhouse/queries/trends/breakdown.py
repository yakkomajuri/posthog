...
from ee.clickhouse.client import sync_execute, substitute_params
...
class ClickhouseTrendsBreakdown:
    ...
    def _format_breakdown_query(
        ...
        if filter.breakdown_type == "cohort":
                _params, breakdown_filter, _breakdown_filter_params, breakdown_value = self._breakdown_cohort_params(
                    breakdown, team_id, filter, entity
                )
        ...
        if filter.display == TRENDS_TABLE or filter.display == TRENDS_PIE:
            ...
            return content_sql, params, self._parse_single_aggregate_result(filter, entity, breakdown)
            ...
            return breakdown_query, params, self._parse_trend_result(filter, entity, breakdown)
    ...
    def _breakdown_cohort_params(self, breakdown, team_id: int, filter: Filter, entity: Entity):
        cohort_queries, cohort_ids, cohort_params = self._format_breakdown_cohort_join_query(breakdown, team_id, filter, entity)
    ...
    def _parse_single_aggregate_result(
        self, filter: Filter, entity: Entity, breakdown: Union[str, List[Union[str, int]], None]
    ) -> Callable:
        def _parse(result: List) -> List:
            parsed_results = []
            for idx, stats in enumerate(result):
                additional_values = self._breakdown_result_descriptors(stats[2], idx, filter, entity, breakdown)
    ...
    def _breakdown_result_descriptors(
        ...
        extra_label = self._determine_breakdown_label(breakdown_value, filter.breakdown_type, breakdown, stripped_value)
    ...
    def _format_all_query(self, team_id: int, filter: Filter, entity: Entity) -> Tuple[str, Dict]:
        parsed_date_from, parsed_date_to, date_params = parse_timestamps(filter=filter, team_id=team_id, table='all_events.')
        ...
        query = """
            ...
            WHERE team_id = {} {} {} {}
            """.format(team_id, parsed_date_from, parsed_date_to, prop_filters)
        return substitute_params(query, {**date_params, **prop_filter_params})

    def _format_breakdown_cohort_join_query(self, breakdown: List[Any], team_id: int, filter: Filter, entity: Entity) -> Tuple[str, List, Dict]:
